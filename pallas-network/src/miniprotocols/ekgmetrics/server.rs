...
impl Server {
    ...
    fn assert_outbound_state(&self, msg: &Message) -> Result<(), ServerError> {
        match (&self.0, msg) {
            (ServerState::Busy, Message::Resp(..)) => Ok(()),
    ...
    fn assert_inbound_state(&self, msg: &Message) -> Result<(), ServerError> {
        match (&self.0, msg) {
            (ServerState::Idle, Message::Req(..)) => Ok(()),
    ...
    pub async fn recv_request(&mut self) -> Result<Option<Request>, ServerError> {
