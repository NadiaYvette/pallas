...
impl Client {
    ...
    fn assert_outbound_state(&self, msg: &Message) -> Result<(), ClientError> {
        match (&self.0, msg) {
            (ClientState::Idle, Message::Request(_)) => Ok(()),
    ...
    fn assert_inbound_state(&self, msg: &Message) -> Result<(), ClientError> {
        match (&self.0, msg) {
            (ClientState::Busy, Message::Response(_)) => Ok(()),
    ...
    pub async fn send_request(&mut self, points: Vec<String>) -> Result<(), ClientError> {
        let msg = Message::Request(points);
        self.send_message(&msg).await?;
        self.0 = ClientState::Busy;
        debug!("sent datapoints request");
        Ok(())
    }

    pub async fn recv_response(&mut self) -> Result<Vec<(String, Option<Vec<u8>>)>, ClientError> {
        let msg = self.recv_message().await?;
        match msg {
            Message::Response(points) => {
                debug!(length = points.len(), "received datapoints response");
