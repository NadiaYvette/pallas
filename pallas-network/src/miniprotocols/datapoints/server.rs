...
impl Server {
    ...
    pub async fn recv_request(&mut self) -> Result<Option<Vec<String>>, ServerError> {
    ...
    pub async fn send_response(
        &mut self,
        points: Vec<(String, Option<Vec<u8>>)>,
    ) -> Result<(), ServerError> {
        let msg = Message::Response(points);
        self.send_message(&msg).await?;
        self.0 = ServerState::Idle;
        debug!("sent datapoints response");
