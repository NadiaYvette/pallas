...
-- | Test trace-proxy as intermediary between node and Haskell cardano-tracer,
--   verifying EKG, DataPoints, and TraceObjects file output.
...
getProxyTracerState TestSetup{..} ref = do
 ...
 where
   setupProxyTracer ekgStore = do
     ...
     -- 1. Start cardano-tracer (Haskell) listening on tracer-real.sock
     ...
     tracerHdl <- Sys.spawnProcess "cardano-tracer"
       [ "--config" , unI tsWorkDir <> "/config.yaml"
       , "--state-dir", unI tsWorkDir <> "/tracer-statedir"
       ]
     threadDelay 10_000_000
     resTracer <- Sys.getProcessExitCode tracerHdl
     case resTracer of
       Nothing   -> putStrLn "cardano-tracer (Haskell) started.."
       Just code -> error $ "cardano-tracer failed to start with code " <> show code

     -- 2. Start trace-proxy listening on nodeSock and connecting to tracerRealSock
     binDir <- fromMaybe "../../../target/release" <$> lookupEnv "PALLAS_BIN_DIR"
     proxyHdl <- Sys.spawnProcess (binDir <> "/trace-proxy")
     ...
     (forwardSink, dpStore) <- withIOManager \iomgr -> do
       ...
       initForwarding iomgr forwardingConf (unI tsNetworkMagic) (Just ekgStore)
         (Just (Net.LocalPipe nodeSock, Initiator))
...
runProxyTest TestSetup{..} _msgCountersRef _msgsRef tracerGetter = ioProperty do
    ...
    mbSubDir <- waitForSubdir logsRootDir (120 :: Int)
    ...
    _ <- waitForFile (logsDir <> "/ekg.json") (120 :: Int)
    _ <- waitForFile (logsDir <> "/datapoints.json") (120 :: Int)
