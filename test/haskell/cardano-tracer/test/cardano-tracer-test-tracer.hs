...
-- | Test that the Rust cardano-tracer clone can receive traces from
--   the Haskell trace-forward library, acting as a drop-in replacement
--   for the Haskell cardano-tracer (minus RTView).
...
main = do
    ...
    ts' <- getTestSetup
             ...
             , tsWorkDir      = Last $ Just $ fromMaybe "/tmp/testRustTracer" mbWorkdir
             }
    ...
    let tracerGetter = getRustTracerState ts tracerRef
...
allTests ts msgCountersRef msgsRef externalTracerGetter =
    testGroup "Tests"
    [ localOption (QuickCheckTests 3) $ testGroup "rust cardano-tracer"
        [ testProperty "multi-threaded forwarder stress test via Rust cardano-tracer" $
            runScriptForwarding ts msgCountersRef msgsRef externalTracerGetter
        ]
    ]

-- | Start the Rust cardano-tracer on tracer.sock.
getRustTracerState ::
     TestSetup Identity
  -> IORef (Maybe (Sys.ProcessHandle, Trace IO Message))
  -> IO (Sys.ProcessHandle, Trace IO Message)
getRustTracerState TestSetup{..} ref = do
  state <- readIORef ref
  case state of
    Just st -> pure st
    Nothing -> do
 ...
 where
   ...
   setupFwdTracer = do
     -- Write config for Rust cardano-tracer listening on tracer.sock
     ...
     -- Start Rust cardano-tracer on tracer.sock
     binDir <- fromMaybe "../../../target/release" <$> lookupEnv "PALLAS_BIN_DIR"
     tracerHdl <- Sys.spawnProcess (binDir <> "/cardano-tracer")
       [ "--config" ,    "config.yaml"
       ]
     threadDelay 2_000_000
     res <- Sys.getProcessExitCode tracerHdl
     case res of
       Nothing   -> putStrLn "Rust cardano-tracer started.."
       Just code ->
         error $ "Rust cardano-tracer failed to start with code " <> show code
     -- Initialize forwarding connecting directly to tracer.sock
     ...
     pure (tracerHdl, forwardTracer (writeToSink forwardSink))
